...
def import_racecard_dataframe(df):
    ...
    if missing:
        raise ValueError(f"Missing racecard columns: {missing}")
    ...
    with connect() as con:
        for _, r in df.iterrows():
            ...
            """, (
                ...
                row.get("distance"), row.get("class"), safe_int(row.get("runners_count")), row.get("ew_terms")
            ))
    ...
    with connect() as con:
        for _, r in df.iterrows():
            ...
            """, (
                ...
                row.get("bookmaker"), safe_float(row.get("decimal_odds")),
